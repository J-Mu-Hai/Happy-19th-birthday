...
function buildLocalAnswer(question) {
  const { knowledgeBase, ranked, best } = selectKnowledgeEntries(question);

  if (!ranked.length) {
    return {
      answer: knowledgeBase.fallbackAnswer || '暂时没有匹配到知识库答案。',
      references: [],
      provider: 'local-kb'
    };
  }

  const answer = best.map((entry) => entry.answer).join('\n\n');
...
function buildHybridLocalAnswer(question, history = []) {
  const { knowledgeBase, ranked, best } = selectKnowledgeEntries(question);

  if (!ranked.length) {
    return {
      answer: buildConversationalFallback(question, history),
      references: [],
      provider: 'local-chat'
    };
  }

  const contextSummary = best
    .map((entry) => String(entry.answer || '').trim())
    .filter(Boolean)
    .slice(0, 2)
    .join('\n\n');

  const preface = /(怎么看|怎么想|为什么|是否|会不会|该不该|能不能)/.test(question)
    ? '如果只基于现在这份内容，我会这样理解：'
    : '我想到的更接近这一层：';

  return {
    answer: `${preface}\n\n${contextSummary || buildConversationalFallback(question, history)}`,
    references: best.map((entry) => entry.title).filter(Boolean),
...
function loadFriendWishes() {
  try {
    ...
    return parsed.map(normalizeFriendWish);
  } catch (error) {
    return [];
  }
}
...
async function generateModelAnswer(question, history = []) {
  ...
  if (!provider) {
    ...
    error.publicMessage = 'Ask Me 还没有配置真实模型。请先设置 OPENAI_API_KEY 或 DEEPSEEK_API_KEY。';
    throw error;
  }

  console.log(`[ask] provider=${provider.name} model=${provider.model} question="${question.slice(0, 80)}"`);
  ...
  const knowledgeContext = buildKnowledgeContext(best);
  const historyText = normalizedHistory.map((item) => `${item.role === 'user' ? '用户' : '助手'}：${item.text}`).join('\n');
...
${knowledgeContext}`
  ...
  try {
    ...
    if (!answer) {
      const error = new Error(`${provider.name} returned empty content`);
      error.statusCode = 502;
      error.publicMessage = '真实模型没有返回有效内容。';
      throw error;
    }
