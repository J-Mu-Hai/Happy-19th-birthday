...
const githubFriendWishesPath = 'data/friend-wishes.json';
const OPENAI_API_URL = 'https://api.openai.com/v1/chat/completions';
const OPENAI_MODEL = process.env.OPENAI_MODEL || 'gpt-4o-mini';
const DEEPSEEK_API_URL = 'https://api.deepseek.com/chat/completions';
const DEEPSEEK_MODEL = process.env.DEEPSEEK_MODEL || 'deepseek-chat';
const DEFAULT_GITHUB_REPO = 'J-Mu-Hai/Happy-19th-birthday';
...
function getGitHubRepoInfo() {
  const fullName = String(process.env.GITHUB_REPO || DEFAULT_GITHUB_REPO).trim();
  const [owner, repo] = fullName.split('/');
  return {
    owner,
    repo,
    branch: String(process.env.GITHUB_BRANCH || DEFAULT_GITHUB_BRANCH).trim() || DEFAULT_GITHUB_BRANCH,
    path: String(process.env.GITHUB_FILE_PATH || githubFriendWishesPath).trim() || githubFriendWishesPath
  };
}
...
function getGitHubHeaders() {
  return {
    ...
    'User-Agent': 'happy-19th-birthday-site'
  };
}

async function fetchGitHubFile() {
  const { owner, repo, branch, path: filePath } = getGitHubRepoInfo();
...
async function loadFriendWishesFromGitHub() {
...
async function saveFriendWishesToGitHub(entries) {
  const normalized = Array.isArray(entries) ? entries.map(normalizeFriendWish) : [];
  const file = await fetchGitHubFile();
  const { owner, repo, branch, path: filePath } = getGitHubRepoInfo();
  const encodedPath = filePath.split('/').map(encodeURIComponent).join('/');
  ...
  const commitName = String(process.env.GITHUB_COMMITTER_NAME || 'Birthday Wall Bot').trim() || 'Birthday Wall Bot';
  const commitEmail = String(process.env.GITHUB_COMMITTER_EMAIL || 'bot@example.com').trim() || 'bot@example.com';
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      message: 'Update friend wishes from website submission',
      content,
      sha: file?.sha,
      branch,
      committer: {
...
async function loadSharedFriendWishes() {
  if (canUseGitHubStorage()) {
    return loadFriendWishesFromGitHub();
  }
...
async function saveSharedFriendWishes(entries) {
  if (canUseGitHubStorage()) {
    return saveFriendWishesToGitHub(entries);
  }
