...
function getBlobAccessModes() {
  const preferred = String(process.env.BLOB_ACCESS || '').trim().toLowerCase();
  const modes = [preferred, 'private', 'public'].filter(Boolean);
  return [...new Set(modes)].filter((mode) => mode === 'private' || mode === 'public');
}

function getBlobDebugInfo() {
  return {
    tokenPresent: Boolean(process.env.BLOB_READ_WRITE_TOKEN),
    accessModes: getBlobAccessModes(),
    pathname: friendWishesBlobPath
  };
}
...
async function loadFriendWishesFromBlob() {
  ...
  for (const access of getBlobAccessModes()) {
    try {
      const result = await get(friendWishesBlobPath, {
        access,
